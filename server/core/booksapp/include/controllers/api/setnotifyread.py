...
def api_setnotifyread_controller(request):

    main_controller = SetNotifyReadController('setNotifyRead', request, True)
    return main_controller.run()


class SetNotifyReadController(BaseController):
    ...
    def run(self):
        ...
        try:
            notifications_for_editing.update(is_read='yes')
